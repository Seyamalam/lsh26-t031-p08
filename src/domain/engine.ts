...
export function gradePointForMark(mark: number): { point: number; band: string } {
...
export function letterForGpa(gpa: number, hasCompulsoryFailure = false): string {
...
export function evaluateStudent(
  ...
  const subjectResults = evaluatedCodes.map((code) => {
    ...
    if (mark === undefined) throw new Error(`${student.id} is missing a mark for ${code}.`)
    return evaluateSubject(subject, mark, code === student.optional)
  })
  const compulsoryResults = subjectResults.filter((result) => !result.isOptional)
  const optionalResult = subjectResults.find((result) => result.isOptional)!
  const compulsoryPointSum = compulsoryResults.reduce((sum, result) => sum + result.point, 0)
...
export function buildCheckingLists(results: StudentResult[]): CheckingLists {
  ...
  results.forEach((result) => {
    const optionalResult = result.subjectResults.find((subject) => subject.isOptional)!
    ...
    const practicalReasons = result.subjectResults
      .filter((subject) => subject.isPractical && subject.practicalPassed === false)
      .map((subject) => `${subject.code}: practical mark is below 8.`)
    if (practicalReasons.length) practical.push(makeEntry(result, practicalReasons))
